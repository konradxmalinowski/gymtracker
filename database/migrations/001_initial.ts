...
import { parseSqlStatements } from './parseSql';
import type { Migration } from './types';

/**
 * The full schema-v1 DDL - byte-identical to `database/schema.sql`.
 *
 * Why this is duplicated rather than read from disk: `database/schema.sql` is the
 * canonical, human-reviewed copy (ARCHITECTURE.md section 7) and is what the Node
 * test harness (`database/node/createTestDatabase.ts`) applies directly via
 * `fs.readFileSync`. This migration also needs the same DDL, but it runs on-device
 * through Metro's JS bundle, which has no filesystem to read an arbitrary `.sql`
 * asset as text without an additional Metro transformer - adding one was judged out
 * of scope/risk for this phase (touches a shared build config file used by every
 * other package). The trade-off is one deliberate copy instead of one build-config
 * change, guarded by `__tests__/database/schema.test.ts`, which reads
 * `schema.sql` from disk and asserts it is byte-for-byte identical to this
 * constant - so the two cannot silently drift apart.
 */
export const SCHEMA_V1_SQL = `-- GymTracker SQLite schema - version 1.
...
export const migration001Initial: Migration = {
  version: 1,
  name: 'initial_schema',
  async up(tx: SqlExecutor): Promise<void> {
    const statements = parseSqlStatements(SCHEMA_V1_SQL);
